                     ...
                     std::vector<std::vector<EFANode *>> frag_cut_plane_nodes)
{
  ...
  // only make new fragments if all the chosen nodes were found otherwise call again looking for other nodes
  if ((edge_cut_count + node_cut_count) != chosen_cut_plane_pair.size())
  {
 //   std::cout << "The Element Node IDs are as follows:\n";
 //   for(unsigned int x = 0; x < fragment_nodes.size(); ++x)
 //   {
 //      auto & this_fragX = fragment_nodes[x]; 
 //      for( unsigned int y = 0; y < this_fragX.size(); ++y)
 //      {
 //        std::cout << this_fragX[y]->id() << "\n"; 
 //      }
 //   }

 //   std::cout << "Chosen node ids are: \n";
 //   std::cout << chosen_cut_plane_pair[0]->id() << "\n";
 //   std::cout << chosen_cut_plane_pair[1]->id() << "\n";
 //   if(1==1)
 //     EFAError("Number of cuts is not equal to the expected number of cuts in EFAFragment2D");
    //all_cut_plane_idx.insert(all_cut_plane_idx.begin(), cut_plane_id);
    //all_cut_plane_nodes.insert(all_cut_plane_nodes.begin(), chosen_cut_plane_pair);
...
EFAFragment2D::branchingSplit(std::map<unsigned int, EFANode *> & EmbeddedNodes)
{
  ...
  for (unsigned int i = 0; i < _boundary_edges.size(); ++i)
  {
    ...
    {
      unsigned int id0(
          _boundary_edges[i]->getIntersection(0, node1) < _boundary_edges[i]->getIntersection(1, node1) ? 0 : 1);
  ...
  for (unsigned int i = 0; i < 3; ++i) // loop over 3 sectors
  {
    ...
    while (add_more_edges)
    {
      ...
      if (_boundary_edges[iedge]->containsNode(three_nodes[iplus1]))
      {
        ...
        new_frag->addEdge(new EFAEdge(_boundary_edges[iedge]->getNode(0), _boundary_edges[iedge]->getNode(1)));
    }
    new_fragments.push_back(new_frag);
  } // i
  return new_fragments;
